            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-8">
                        ...
                        <tbody id="detection-table-body">
                            <?php if (count($markers) > 0 || count($missing_coordinates) > 0): ?>
                                <?php foreach ($markers as $marker): ?>
                                    <tr id="row-<?= safe_htmlspecialchars($marker['id']) ?>" class="<?= $marker['elephant'] ? 'highlighted-row bg-red-500' : ($marker['alert'] ? 'bg-green-500' : '') ?>">
                                        <td class="border px-4 py-2"><?= safe_htmlspecialchars($marker['id']) ?></td>
                                        <td class="border px-4 py-2"><?= safe_htmlspecialchars($marker['timestamp']) ?></td>
                                        <td class="border px-4 py-2"><?= safe_htmlspecialchars($marker['lat_cam']) ?>, <?= safe_htmlspecialchars($marker['long_cam']) ?></td>
                                        <td class="border px-4 py-2"><?= safe_htmlspecialchars($marker['lat_ele']) ?>, <?= safe_htmlspecialchars($marker['long_ele']) ?></td>
                                        <td class="border px-4 py-2"><?= safe_htmlspecialchars($marker['distance_ele']) ?> m</td>
                                        <td class="border px-4 py-2">
                                            <?php if (!empty($marker['image_path'])): ?>
                                                <button onclick="openImageModal('<?= safe_htmlspecialchars($marker['image_path']) ?>')" class="bg-purple-500 text-white px-3 py-1 rounded">View Image</button>
                                            <?php else: ?>
                                                <span class="text-gray-500">No Image</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="border px-4 py-2">
                                            <button onclick="focusOnMarker(<?= safe_htmlspecialchars($marker['id']) ?>, 'cam')" class="bg-blue-500 text-white px-3 py-1 rounded">Focus Camera</button>
                                            <?php if ($marker['elephant']): ?>
                                                <button onclick="focusOnMarker(<?= safe_htmlspecialchars($marker['id']) ?>, 'ele')" class="bg-green-500 text-white px-3 py-1 rounded">Focus Elephant</button>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                ...
                                <?php foreach ($missing_coordinates as $marker): ?>
                                    <tr id="row-<?= safe_htmlspecialchars($marker['id']) ?>" class="bg-yellow-500 text-white">
                                        <td class="border px-4 py-2"><?= safe_htmlspecialchars($marker['id']) ?></td>
                                        <td class="border px-4 py-2"><?= safe_htmlspecialchars($marker['timestamp']) ?></td>
                                        <td class="border px-4 py-2">
                ...
                <div class="flex justify-between items-center mt-6">
                    ...
                    <div class="flex space-x-2">
                        <?php if ($page > 1): ?>
                            <a href="?page=<?= safe_htmlspecialchars($page - 1) ?>" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Prev</a>
                        <?php endif; ?>
                        <?php if ($page < $total_pages): ?>
                            <a href="?page=<?= safe_htmlspecialchars($page + 1) ?>" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Next</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function initializeMap() {
        ...
        // เพิ่มมาร์กเกอร์สำหรับการตรวจจับที่มีพิกัดเป็น null (ไม่เพิ่มลงบนแผนที่)
        // คุณสามารถเพิ่มฟีเจอร์อื่น ๆ ที่เกี่ยวข้องกับข้อมูลที่ขาดหายไปได้ที่นี่
    }
    ...
    function checkTimeout() {
        setInterval(() => {
            ...
            if (Date.now() - lastDetectionTime > 10000) {
                ...
                // ไม่ต้องซ่อน Header Alert เพราะต้องให้มันคงอยู่จนกว่าจะได้รับค่าใหม่
            }
    ...
    function showHeaderAlert(message, popupColorClass) {
        ...
        let alertDiv = document.getElementById("headerAlert");
        if (alertDiv) {
            // ถ้ามีอยู่แล้ว ให้เปลี่ยนข้อความและคลาสสี
            alertDiv.textContent = message;
            alertDiv.className = `alert-header ${popupColorClass}`;
        } else {
            // ถ้ายังไม่มี ให้สร้างใหม่
            alertDiv = document.createElement("div");
            alertDiv.id = "headerAlert";
            alertDiv.className = `alert-header ${popupColorClass}`;
            alertDiv.textContent = message;
            document.querySelector("header").prepend(alertDiv);
        }
        // ไม่ตั้งเวลาให้ลบการแจ้งเตือน เพื่อให้คงอยู่จนกว่าจะมีการแจ้งเตือนใหม่
    }
            ...
            if (data && data.status === 'success' && Array.isArray(data.data)) {
                if (data.data.length > 0) {
                    ...
                    data.data.forEach(detection => {
                        if (!markersObject[detection.id + '_cam']) { // ตรวจสอบว่ามาร์กเกอร์ยังไม่ถูกเพิ่ม
                            addMarkerToMap(detection);
                            addDetectionToTable(detection);
                        }
                    });
                    // แสดง Popup ตามการตรวจจับล่าสุด
                    const latestDetection = data.data[0];
                    handleNewDetection(latestDetection);
                } else {
                    // ถ้าไม่มีการตรวจจับ ให้ซ่อน Popup และ Header Alert
                    hidePopup();
                    hideHeaderAlert();
                }
            }
    ...
    function addDetectionToTable(detection) {
        ...
        newRow.innerHTML = `
            ...
            <td class="border px-4 py-2">${detection.lat_cam !== null && detection.long_cam !== null ? `${safe_htmlspecialchars(detection.lat_cam)}, ${safe_htmlspecialchars(detection.long_cam)}` : `<span class="text-red-800">Camera coordinates missing</span>`}</td>
            <td class="border px-4 py-2">${detection.lat_ele !== null && detection.long_ele !== null ? `${safe_htmlspecialchars(detection.lat_ele)}, ${safe_htmlspecialchars(detection.long_ele)}` : `<span class="text-red-800">Elephant coordinates missing</span>`}</td>
