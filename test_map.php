...
<html lang="th">
  ...
  <body>
    ...
    <script>
      ...
      const latParam = parseFloat("<?php echo htmlspecialchars($lat); ?>");
      const lngParam = parseFloat("<?php echo htmlspecialchars($lng); ?>");

      // ฟังก์ชันสำหรับเริ่มต้นแผนที่
      function initializeMap() {
        // กำหนดพิกัดเริ่มต้น
        let initialLat = 14.439606;
        let initialLng = 101.372359;
        let initialZoom = 13;

        // ถ้ามีพารามิเตอร์ lat และ lng จาก URL ให้ใช้พิกัดนั้น
        ...
        // สร้างแผนที่
        mymap = L.map("mapid").setView([initialLat, initialLng], initialZoom);

        // เพิ่มแผนที่พื้นฐานจาก OpenStreetMap หรือ Dark Mode
        currentTileLayer = L.tileLayer(isDarkMode ? darkTileLayer : lightTileLayer, {
          maxZoom: 19,
          attribution: "© OpenStreetMap",
        }).addTo(mymap);

        // ถ้ามีพารามิเตอร์ lat และ lng จาก URL ให้เพิ่ม marker ตามพารามิเตอร์
        if (!isNaN(latParam) && !isNaN(lngParam)) {
          L.marker([latParam, lngParam])
            .addTo(mymap)
            .bindPopup(
              `${type === "cam" ? "กล้อง CCTV" : "ช้าง"} ID: ${id}`
            )
            .openPopup();
        }

        // เพิ่มกล้อง CCTV และวงกลมรัศมีลงบนแผนที่ (เดิม)
